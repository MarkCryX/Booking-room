...
import Image from "next/image"; // Import Image component for displaying QR codes
...
function AtvBookingList({ bookings, setBookings }) {
  const [modalOpen, setModalOpen] = useState(false);
  const [showBankModal, setShowBankModal] = useState(false);
  const [showQrcodeModal, setShowQrcodeModal] = useState(false); // New state for QR modal
  const [bankAccount, setBankAccount] = useState([]);
  const [imagePayment, setImagePayment] = useState("");
  const [currentBookingId, setCurrentBookingId] = useState(null);
  const [totalPrice, setTotalPrice] = useState(0);
  const [copiedAccount, setCopiedAccount] = useState(null); // State to track copied account for bank modal

  // Effect to manage body overflow when modals are open/closed
  useEffect(() => {
  ...
  }, [modalOpen, showBankModal, showQrcodeModal]);
  ...
  const handleSubmitBankPayment = async () => {
    ...
    try {
      const response = await fetch("/api/user/payment/atv", {
        // Use the ATV-specific API
        method: "PATCH",
        headers: {
  ...
  const handleDelete = async (bookingId) => {
    try {
      ...
      if (response.ok) {
        alert("ลบการจอง ATV สำเร็จ");
        setBookings((prevBookings) =>
          prevBookings.filter((booking) => booking._id !== bookingId)
        );
      } else {
        alert("เกิดข้อผิดพลาดในการยกเลิกการจอง ATV");
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchBankAccount = async () => {
      try {
        const response = await fetch("/api/bankaccounts"); // Assuming this API provides bank accounts for ATV
      ...
      {bookings.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500 mt-1">
            คุณยังไม่มีรายการจอง ATV ในขณะนี้
          </p>
        </div>
      ) : (
          ...
          {bookings.map((booking) => (
            ...
            >
              ...
              <div className="space-y-3 text-gray-700">
                ...
                <div className="flex items-center">
                  <FontAwesomeIcon
                    ...
                    className="h-4 w-4 text-blue-500 mr-2"
                  />
                  <span>{booking.selectedRound} น.</span>
                </div>
                <div className="flex items-center">
                  <FontAwesomeIcon
                    icon={faRightLeft}
                    className="h-4 w-4 text-blue-500 mr-2"
                  />
                  <span>ATV ขนาดเล็ก: {booking.numSmallATV} คัน</span>
                </div>
                <div className="flex items-center">
                  <FontAwesomeIcon
                    icon={faRightLeft}
                    className="h-4 w-4 text-blue-500 mr-2"
                  />
                  <span>ATV ขนาดใหญ่: {booking.numLargeATV} คัน</span>
                </div>
              </div>
              ...
              {booking.paymentStatus === "ยังไม่ชำระ" && (
                <div>
                  <button
                    className="w-full mt-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-3 px-4 rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-colors duration-300 ease-in-out font-semibold text-lg shadow-md flex items-center justify-center"
                    onClick={() =>
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <h3 className="text-xl font-bold mb-4">ชำระเงินสำหรับ ATV</h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            <div className="flex items-center justify-between ">
              ...
              <p className="text-xl font-bold mb-4 bg-blue-100 py-2 px-3 rounded-full">
            ...
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
              <div className="space-y-1 text-center">
                <div className="flex text-sm text-gray-600">
                  <label
                    htmlFor="file-upload-bank-atv"
                    className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none"
                  >
                    <span>อัพโหลดสลิป</span>
                    <input
                      id="file-upload-bank-atv"
                      name="file-upload-bank-atv"
                      type="file"
                      accept="image/*"
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg">
              <div className="space-y-1 text-center">
                <div className="flex text-sm text-gray-600">
                  <label
                    htmlFor="file-upload-qr-atv"
                    className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 focus-within:outline-none"
                  >
                    <span>อัพโหลดสลิป</span>
                    <input
                      id="file-upload-qr-atv"
                      name="file-upload-qr-atv"
                      type="file"
                      accept="image/*"
                      onChange={handleImageChange}
                      className="sr-only"
