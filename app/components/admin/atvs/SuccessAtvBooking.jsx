...
import { FiSearch, FiCalendar, FiX, FiDollarSign, FiUser, FiPhone, FiMail, FiCheckCircle, FiClock } from "react-icons/fi";

function SuccessAtvBooking() {
  ...
  const filteredBookings = atvbooking.filter((booking) => {
    const nameMatch = booking.name.toLowerCase().includes(searchTerm.toLowerCase());
    const dateMatch = dateSearch ?
      new Date(booking.selectedDate).toLocaleDateString('en-CA') === dateSearch :
      true;
    ...
    <div className="container mx-auto p-4 max-w-7xl">
      ...
      {filteredBookings.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-8 text-center">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">ไม่พบข้อมูลการจอง</h3>
          <p className="text-gray-500">ไม่มีออเดอร์ที่สำเร็จตามเงื่อนไขการค้นหาของคุณ</p>
        </div>
      ) : (
        <>
                ...
                <div className="p-5">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center text-gray-600">
                      ...
                      <span>{new Date(Booking.selectedDate).toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
                    </div>
                    <div className="flex items-center text-gray-600">
                      <FiClock className="mr-2 text-blue-500" />
                      <span>{Booking.selectedRound}</span>
                    </div>
                    <div className="flex items-center text-gray-600">
                      <span className="w-6 mr-2">🛵</span>
                      <span>คันเล็ก: {Booking.numSmallATV} คัน</span>
                    </div>
                    <div className="flex items-center text-gray-600">
                      <span className="w-6 mr-2">🏍️</span>
                      <span>คันใหญ่: {Booking.numLargeATV} คัน</span>
                    </div>
                    <div className="flex items-center text-gray-600">
                      <FiDollarSign className="mr-2 text-blue-500" />
                      <span className="font-medium">ราคารวม: {Booking.totalPrice.toLocaleString()} บาท</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                      ...
                      {Booking.status === "รอยืนยัน" ? <FiClock className="mr-1" /> : <FiCheckCircle className="mr-1" />}
                      {Booking.status}
                    </div>
                    ...
                    <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ml-2 ${
                      ...
                      {Booking.paymentStatus === "ยังไม่ชำระ" ? "ยังไม่ชำระ" : Booking.paymentStatus}
                    </div>
                  </div>
...
export default SuccessAtvBooking;